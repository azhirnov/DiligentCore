...
template <typename BaseInterface, typename ImplementationTraits>
bool DeviceContextBase<BaseInterface, ImplementationTraits>::TraceRaysIndirect(const TraceRaysIndirectAttribs& Attribs, IBuffer* pAttribsBuffer, int) const
{
#ifdef DILIGENT_DEVELOPMENT
    ...
    {
        LOG_ERROR_MESSAGE("IDeviceContext::TraceRaysIndirect command uses resource state transition and must be performed outside of render pass");
        return false;
    }

    if (!VerifyTraceRaysIndirectAttribs(m_pDevice, Attribs, pAttribsBuffer))
        return false;
